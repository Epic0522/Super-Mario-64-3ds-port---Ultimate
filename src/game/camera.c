...
static void sync_lakitu_state_to_camera(struct Camera *c) {
...
void update_camera(struct Camera *c) {
    ...
    if (c->cutscene == 0) {
        ...
        if (cam_select_alt_mode(0) == CAM_SELECTION_MARIO && c->mode != CAM_MODE_NEWCAM && !newcam_active) {
    ...
    } else if (newcam_active && c->cutscene == 0) {
        c->mode = CAM_MODE_NEWCAM;
        c->defMode = CAM_MODE_NEWCAM;
        gLakituState.mode = CAM_MODE_NEWCAM;
        gLakituState.defMode = CAM_MODE_NEWCAM;
