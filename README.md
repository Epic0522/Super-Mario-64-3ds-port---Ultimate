...
在开发过程中，少量“生成资产侧”文件被有意补丁化，用于本项目的行为或表现修正。由于 `extract_assets.py` 可能会覆盖这些文件，项目会在 `project_asset_overrides/` 下保留一份镜像替换包。本地完成资产提取后，需要把该目录内容再复制回仓库根目录，让补丁版文件覆盖新生成的版本：
...
## Development Screenshots / 开发截图

The `indevscreenshots/` directory contains archived in-development screenshots collected during the project and is kept in the repository as historical reference material.

`indevscreenshots/` 目录保存了项目开发期间归档下来的截图，用作开发过程记录与历史参考。
